...
function handleSearchQuery(event) {
  ...
  if (!query) {
    displayErrorMessage("Por favor introduzca una palabra");
    return;
  }
  const results = searchSite(query);
  if (!results.length) {
    displayErrorMessage("No logramos encontrar lo que buscabas");
    return;
  }
...
function updateSearchResults(query, results) {
  ...
  document.getElementById("results-count-text").innerHTML =
    searchResultListItems.length > 1 ? "resultados" : "resultado";
